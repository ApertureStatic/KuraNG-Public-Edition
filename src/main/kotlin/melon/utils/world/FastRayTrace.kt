...
val function: World.(BlockPos, BlockState) -> FastRayTraceAction = { pos, blockState ->
    if (blockState.getOutlineShape(this, pos) != null) {
...
inline fun World.fastRaytrace(
    ...
    while (count-- >= 0) {
        ...
        mutableBlockPos.set(currentBlockX, currentBlockY, currentBlockZ)
        if (isOutOfHeightLimit(mutableBlockPos) || !worldBorder.contains(mutableBlockPos)) continue
...
fun BlockState.fastRaytrace(
    ...
    if (!this.getOutlineShape(world, blockPos).isEmpty) {
        val box = this.getOutlineShape(world, blockPos).boundingBox
