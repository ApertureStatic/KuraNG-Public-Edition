...
class ModuleButton(val module: dev.dyzjct.kura.module.AbstractModule, width: Double, height: Double, father: Panel) :
    Component() {
    private var fade = 0
    ...
    private fun setup() {
        for (value in module.settingList) {
            when (value) {
                ...
                is ColorSetting -> settings.add(ColorPickerUtil(value, father, width, height))
            }
        }
